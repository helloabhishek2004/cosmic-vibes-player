...
export function SongCard({ song, onDownload, playlist }: { song: Song; onDownload: (s: Song) => void; playlist?: Song[] }) {
  ...
  const rememberSong = () => {
    try { sessionStorage.setItem(SELECTED_SONG_KEY, JSON.stringify(song)); } catch {}
  };
  ...
  return (
    <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className={`glass rounded-3xl p-4 group transition-shadow ${active && status === "playing" ? "shadow-[0_20px_60px_-20px_rgba(0,212,255,0.7)]" : "hover:shadow-[0_20px_50px_-20px_rgba(123,111,240,0.5)]"}`}>
      <Link to="/song/$id" params={{ id: song.id }} onClick={rememberSong} className="block" aria-label={`View ${song.title} by ${song.artist}`}>
        <div className="relative aspect-square rounded-2xl overflow-hidden mb-4">
          ...
          {active && status === "playing" && <div className="absolute bottom-3 left-3 flex items-end gap-0.5 h-6" aria-hidden><span className="eq-bar" /><span className="eq-bar" /><span className="eq-bar" /><span className="eq-bar" /></div>}
        </div>
      ...
      <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => onDownload(song)} aria-label={`Download ${song.title}`} className="mt-3 w-full py-2.5 rounded-full glass hover:bg-white/10 flex items-center justify-center gap-2 text-sm font-medium transition">
