...
export function downloadAudio(videoId, outputDir, onProgress, metadata) {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      "--cache-dir", path.join(outputDir, ".cache"),
      "--no-live",
      "--extract-audio",
      "--audio-format", "mp3",
      "--audio-quality", "0",
      ...
      // YouTube currently requires an EJS challenge solver. Node is supplied by
      // the same container as the backend and is a supported yt-dlp JS runtime.
      "--js-runtimes", "node",
    ];

    if (cookiesPath) {
      args.push("--cookies", cookiesPath);
      // Do not use iOS here: yt-dlp explicitly rejects cookies for that client.
      // The web client can use the supplied browser cookies and EJS challenge solver.
